...
export type DeletionOrigin = 'local' | 'cli'
...
export interface DeletedFileRecord {
  ...
  category: string
  /**
   * Surface that triggered the deletion. A CLI-triggered clean can overlap a
...
export interface ErtCleanerSettings {
...
/** Package managers ErtCleaner can aggregate on Windows. */
export type WindowsPackageManager = 'winget' | 'choco' | 'scoop' | 'npm'

/** Package-manager names ErtCleaner can report. Windows-only. */
export type PackageManagerName = WindowsPackageManager
...
export type TrimSupport = 'supported' | 'disabled' | 'unsupported'
