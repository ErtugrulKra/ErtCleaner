...
describe('validateSettingsPartial', () => {
  ...
  it('strips legacy cloud-agent settings instead of rejecting the payload', () => {
    expect(validateSettingsPartial({
      theme: 'dark',
      cloud: { apiKey: 'secret', allowRemotePower: true },
    })).toEqual({ theme: 'dark' })
  })
...
describe('validateDeletionQuery', () => {
  ...
  it('accepts known origins and rejects anything else', () => {
    for (const origin of ['local', 'cli']) {
      expect(validateDeletionQuery({ origin })).toEqual({ origin })
    }
    expect(validateDeletionQuery({ origin: 'cloud' })).toBeNull()
