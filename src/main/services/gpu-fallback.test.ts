...
describe('gpu-fallback', () => {
  beforeEach(() => {
    h.dataDir.value = mkdtempSync(join(tmpdir(), 'ertcleaner-gpu-'))
    h.switches.length = 0
    for (const k of Object.keys(h.listeners)) delete h.listeners[k]
    h.relaunch.mockClear()
    h.exit.mockClear()
    process.argv = ['node', 'ertcleaner']
    delete process.env.ERTCLEANER_DISABLE_GPU
  })
  ...
  it('honors the --disable-gpu flag', async () => {
    process.argv = ['node', 'ertcleaner', '--disable-gpu']
    const { shouldDisableGpu } = await load()
    expect(shouldDisableGpu()).toBe(true)
  })

  it('honors the ERTCLEANER_DISABLE_GPU env var', async () => {
    process.env.ERTCLEANER_DISABLE_GPU = '1'
    const { shouldDisableGpu } = await load()
    expect(shouldDisableGpu()).toBe(true)
  })
