...
import type { ErtCleanerSettings, ScheduleEntry, ScheduleTaskType, MalwareAllowlistEntry, WindowsPackageManager, WindowState } from '../../shared/types'
...
function readStore(): StoreData {
  ...
  try {
    if (existsSync(getConfigPath())) {
      ...
      // Drop leftover cloud-agent settings from older installs. The remote
      // agent was never shipped; persisting allowRemotePower / apiKey would
      // only confuse a source review.
      if (merged.settings && 'cloud' in merged.settings) {
        delete (merged.settings as { cloud?: unknown }).cloud
        try { writeStore(merged) } catch (err) { logError('Cloud-settings strip write failed', err) }
      }
...
export function getSettings(): ErtCleanerSettings {
...
export function setSettings(partial: Partial<ErtCleanerSettings>): void {
