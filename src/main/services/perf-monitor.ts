...
export class PerfMonitorService {
    ...
    try {
      ...
      try {
        ...
        return {
          success: false,
          error: requiresAdmin
            ? 'Erişim reddedildi. Bu işlemi sonlandırmak için ErtCleaner\'ı yönetici olarak çalıştırın.'
  ...
  private async collectSnapshot(): Promise<void> {
    ...
    try {
      ...
      // si.mem() costs ~290ms per call on Windows — os.totalmem()/os.freemem()
      // return the same values at near-zero cost.
