...
describe('win32 paths', () => {
  ...
  describe('systemCleanTargets', () => {
    ...
    it('includes user temp files under LOCALAPPDATA\\Temp', () => {
      const userTemp = targets.find((t) => t.subcategory === 'Kullanıcı Geçici Dosyaları')
    ...
    it('includes system temp files under WINDIR\\Temp', () => {
      const sysTemp = targets.find((t) => t.subcategory === 'Sistem Geçici Dosyaları')
      expect(sysTemp).toBeDefined()
      expect(sysTemp!.path).toBe('C:\\Windows\\Temp')
    })

    it('marks system-level targets as needsAdmin', () => {
      const prefetch = targets.find((t) => t.subcategory === 'Prefetch Verileri')
    ...
    it('includes Windows Update Cache and Delivery Optimization', () => {
      const wuCache = targets.find((t) => t.subcategory === 'Windows Update Önbelleği')
      const doCach = targets.find((t) => t.subcategory === 'Teslimat Optimizasyonu Önbelleği')
    ...
    it('includes administrator-only Update Orchestrator logs', () => {
      const usoLogs = targets.find((t) => t.subcategory === 'Güncelleme Düzenleyicisi Günlükleri')
    ...
    it('includes Previous Windows Installation (Windows.old)', () => {
      const old = targets.find((t) => t.subcategory === 'Önceki Windows Kurulumu')
  ...
  describe('singleFileCleanTargets', () => {
    it('includes the full memory dump file', () => {
      ...
      expect(dumpTarget!.subcategory).toBe('Tam Bellek Dökümü')
    })
  })
  ...
  describe('appPaths', () => {
    ...
    it('includes the added Windows cache coverage', () => {
      ...
      expect(ids).toEqual(expect.arrayContaining(['electron', 'ffmpeg-static', 'google-drive', 'ertcleaner', 'webview2', 'zed']))
