...
function printHelp(): void {
  log(`
ErtCleaner CLI — Full-featured command line interface

Usage:
  ertcleaner --cli <command> [subcommand] [options]
...
CVE Scanner:
  cve list                   List known CVE vulnerabilities (disabled)
...
Config Management:
  config get [key]             Show settings (e.g. config get language)
  config set <key> <value>     Update a setting (e.g. config set language tr)
...
Examples:
  ertcleaner --cli scan --all --clean        Scan & clean all file categories
  ertcleaner --cli registry scan --json      Scan registry, JSON output
  ertcleaner --cli debloat scan              List removable bloatware
  ertcleaner --cli startup list              Show startup items
  ertcleaner --cli malware scan              Run malware scan
  ertcleaner --cli perf info                 Show system specs
  ertcleaner --cli metrics                   Print Prometheus metrics
  ertcleaner --cli metrics-server --port 9200  Start metrics endpoint
`.trim())
}
...
async function handleRegistry(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    cliUsage(ctx, 'ertcleaner --cli registry <scan|fix> [--all] [--json]')
...
async function handleStartup(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'list') {
    ...
    if (!name) { cliUsage(ctx, `ertcleaner --cli startup ${sub} <name>`); return ExitCode.INVALID_ARGS }
    const items = await listStartupItems()
    ...
    const name = args.slice(1).join(' ')
    if (!name) { cliUsage(ctx, 'ertcleaner --cli startup delete <name>'); return ExitCode.INVALID_ARGS }
    const items = await listStartupItems()
    const item = items.find(i => i.name === name || i.displayName === name)
    if (!item) { cliNotFound(ctx, 'Startup item', name); return ExitCode.NOTHING_FOUND }
    ...
    cliUsage(ctx, 'ertcleaner --cli startup <list|boot-trace|disable|enable|delete> [name]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleDebloat(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    if (allFlag) {
      ...
      if (!pkgArg) { cliUsage(ctx, 'ertcleaner --cli debloat remove <pkg1,pkg2,...> or --all'); return ExitCode.INVALID_ARGS }
    ...
    cliUsage(ctx, 'ertcleaner --cli debloat <scan|remove> [packages|--all]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleDisk(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'drives') {
    ...
    if (!drive) { cliUsage(ctx, 'ertcleaner --cli disk analyze <drive-letter>'); return ExitCode.INVALID_ARGS }
    ...
    const drive = args[1]?.replace(':', '')
    if (!drive) { cliUsage(ctx, 'ertcleaner --cli disk file-types <drive-letter>'); return ExitCode.INVALID_ARGS }
    ...
    cliUsage(ctx, 'ertcleaner --cli disk <drives|analyze|file-types> [drive-letter]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleNetwork(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    cliUsage(ctx, 'ertcleaner --cli network <scan|clean> [--all]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleMalware(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    if (!path) { cliUsage(ctx, 'ertcleaner --cli malware quarantine <path>'); return ExitCode.INVALID_ARGS }
    const result = await quarantineMalware([path])
    cliOut(ctx, result)
  } else if (sub === 'delete') {
    const path = args.slice(1).filter(a => !a.startsWith('--')).join(' ')
    if (!path) { cliUsage(ctx, 'ertcleaner --cli malware delete <path>'); return ExitCode.INVALID_ARGS }
    const result = await deleteMalware([path])
    cliOut(ctx, result)
  } else {
    cliUsage(ctx, 'ertcleaner --cli malware <scan|quarantine|delete> [path]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handlePrivacy(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    cliUsage(ctx, 'ertcleaner --cli privacy <scan|apply> [--all]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleDrivers(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    if (!nameArg) { cliUsage(ctx, 'ertcleaner --cli drivers clean <name1,name2,...>'); return ExitCode.INVALID_ARGS }
    ...
    if (toInstall.length === 0) { cliUsage(ctx, 'ertcleaner --cli drivers update <id,...> or --all'); return ExitCode.INVALID_ARGS }
    ...
    cliUsage(ctx, 'ertcleaner --cli drivers <scan|clean|check-updates|update> [names|--all]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleServices(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan') {
    ...
    if (!name) { cliUsage(ctx, `ertcleaner --cli services ${sub} <service-name>`); return ExitCode.INVALID_ARGS }
    ...
    cliUsage(ctx, 'ertcleaner --cli services <scan|disable|enable|manual|auto> [service-name]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handlePrograms(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'list') {
    ...
    cliUsage(ctx, 'ertcleaner --cli programs list')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleUpdates(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'check') {
    ...
    if (items.length === 0) { cliUsage(ctx, 'ertcleaner --cli updates run <id,...> or --all'); return ExitCode.INVALID_ARGS }
    ...
    cliUsage(ctx, 'ertcleaner --cli updates <check|run> [ids|--all]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handlePerf(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'info') {
    ...
    if (isNaN(pid)) { cliUsage(ctx, 'ertcleaner --cli perf kill <pid>'); return ExitCode.INVALID_ARGS }
    const result = await perf.killProcess(pid)
    cliOut(ctx, result)
  } else {
    cliUsage(ctx, 'ertcleaner --cli perf <info|disk-health|kill> [pid]')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleLeftovers(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'scan' || sub === 'clean') {
    ...
    cliUsage(ctx, 'ertcleaner --cli leftovers <scan|clean>')
    return ExitCode.INVALID_ARGS
  }
}

async function handleCve(_args: string[], ctx: CliContext): Promise<number | void> {
  cliOut(ctx, ctx.json ? { error: 'CVE taraması devre dışı' } : 'CVE taraması devre dışı.')
...
async function handleHistory(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  if (sub === 'list') {
    ...
    cliUsage(ctx, 'ertcleaner --cli history <list|clear>')
    return ExitCode.INVALID_ARGS
  }
}
...
async function handleRestorePoint(args: string[], ctx: CliContext): Promise<number | void> {
  ...
  const description = args.slice(1).filter(a => !a.startsWith('--')).join(' ') || 'ErtCleaner CLI restore point'
  ...
  if (args[0] === 'create') {
    ...
    cliUsage(ctx, 'ertcleaner --cli restore-point create [description]')
    return ExitCode.INVALID_ARGS
  }
}
  ...
  if (sub === 'get') {
    ...
    if (!key || !rawValue) {
      if (ctx.json) {
        ...
        cliLog(ctx, 'Usage: ertcleaner --cli config set <key> <value>')
        cliLog(ctx, 'Example: ertcleaner --cli config set language tr')
      }
      return ExitCode.INVALID_ARGS
    }
    ...
    await flushSettings()
    if (!ctx.json) cliLog(ctx, `  Set ${key} = ${value}`)
    else cliOut(ctx, { success: true, key, value })
  } else {
    if (ctx.json) {
      cliOut(ctx, { error: 'invalid_usage', usage: 'config <get|set> [key] [value]' })
    } else {
      cliLog(ctx, 'Usage: ertcleaner --cli config <get|set> [key] [value]')
      cliLog(ctx, '')
      cliLog(ctx, 'Examples:')
      cliLog(ctx, '  ertcleaner --cli config get                        Show all settings')
      cliLog(ctx, '  ertcleaner --cli config get language               Show language')
      cliLog(ctx, '  ertcleaner --cli config set language tr            Set language')
    }
    return ExitCode.INVALID_ARGS
  }
}

// ─── systemd service (removed; ErtCleaner is Windows-only) ──

async function handleService(_args: string[], ctx: CliContext): Promise<number> {
  if (ctx.json) {
    cliOut(ctx, { error: 'unsupported_command', message: 'systemd service management is not available on Windows' })
  } else {
    log('Error: systemd service management is not available. ErtCleaner is Windows-only.')
  }
  return ExitCode.INVALID_ARGS
}
...
async function runLegacyScanClean(categories: string[], doClean: boolean, ctx: CliContext): Promise<number> {
  ...
  cliLog(ctx, `ErtCleaner CLI v${app.getVersion()}`)
...
export async function runCli(): Promise<void> {
  ...
  if (parsed.version) { log(`ErtCleaner v${app.getVersion()}`); app.exit(ExitCode.SUCCESS); return }
    ...
    switch (parsed.command) {
      ...
      default:
        if (ctx.json) log(JSON.stringify({ error: 'unknown_command', command: parsed.command }))
          ...
          log('Run ertcleaner --cli --help for usage information.')
        }
