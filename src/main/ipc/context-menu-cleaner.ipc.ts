...
async function applyOne(
  ...
  if (entry.requiresAdmin && !isAdmin()) {
    return { ok: false, reason: 'Access denied — run ErtCleaner as administrator.' }
  }
...
function cleanRegError(message: string): string {
  ...
  if (/access is denied/i.test(message)) return 'Access denied — run ErtCleaner as administrator.'
