...
vi.mock('electron', () => ({
  app: {
    isPackaged: false,
    getPath: (name: string) => {
      if (name === 'userData') return 'C:\\Users\\Test\\AppData\\Roaming\\ErtCleaner'
...
describe('stale startup entries', () => {
  ...
  describe('stale flag', () => {
    ...
    it('expands environment variables when resolving the target', async () => {
      process.env.__ERTCLEANER_TEST_APPDIR = 'C:\\Apps'
      onlyExisting('C:\\Apps\\present.exe')
      setupExecFileHandler((cmd, args) => {
        if (cmd === 'reg' && args[1] === HKCU_RUN) {
          return {
            stdout: regOutput([
              { name: 'Present', command: '%__ERTCLEANER_TEST_APPDIR%\\present.exe' },
              { name: 'Gone', command: '%__ERTCLEANER_TEST_APPDIR%\\gone.exe' },
              { name: 'UnknownVar', command: '%__ERTCLEANER_NOT_SET__%\\app.exe' },
            ])
          }
        }
        throw new Error('not found')
      ...
      delete process.env.__ERTCLEANER_TEST_APPDIR
    })
  })
})
