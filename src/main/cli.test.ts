...
describe('parseCliArgs', () => {
  it('parses --json flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--json', 'registry', 'scan'])
    expect(result.ctx.json).toBe(true)
    expect(result.command).toBe('registry')
  })

  it('parses --verbose flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--verbose', 'scan'])
    expect(result.ctx.verbosity).toBe('verbose')
  })

  it('parses --quiet flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--quiet', 'scan'])
    expect(result.ctx.verbosity).toBe('quiet')
  })

  it('parses -q as quiet', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '-q', 'scan'])
    expect(result.ctx.verbosity).toBe('quiet')
  })

  it('defaults to normal verbosity', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', 'scan'])
    expect(result.ctx.verbosity).toBe('normal')
  })

  it('parses --help flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--help'])
    expect(result.help).toBe(true)
  })

  it('parses -h flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '-h'])
    expect(result.help).toBe(true)
  })

  it('parses --version flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--version'])
    expect(result.version).toBe(true)
  })

  it('parses -v flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '-v'])
    expect(result.version).toBe(true)
  })

  it('extracts command correctly', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', 'malware', 'scan', '--json'])
    expect(result.command).toBe('malware')
    expect(result.ctx.json).toBe(true)
  })

  it('filters global flags from commandArgs', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--json', '--verbose', 'debloat', 'remove', '--all'])
  ...
  it('detects legacy flags', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--system', '--browser'])
    expect(result.hasLegacyFlags).toBe(true)
    expect(result.command).toBeUndefined()
  })

  it('detects --all as legacy flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--all'])
    expect(result.hasLegacyFlags).toBe(true)
  })

  it('detects --clean flag', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli', '--all', '--clean'])
    expect(result.hasCleanFlag).toBe(true)
  })

  it('handles no arguments after --cli', () => {
    const result = parseCliArgs(['node', 'ertcleaner', '--cli'])
    expect(result.command).toBeUndefined()
