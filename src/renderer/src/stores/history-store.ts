...
export const useHistoryStore = create<HistoryState>((set, get) => ({
  ...
  load: async () => {
    try {
      const entries = await window.ertcleaner.historyGet()
  ...
  addEntry: async (entry) => {
    try {
      await window.ertcleaner.historyAdd(entry)
  ...
  clear: async () => {
    try {
      await window.ertcleaner.historyClear()
      set({ entries: [] })
    } catch {
      // Silent fail
    }
...
if (!_historyListenerRegistered) {
  ...
  window.ertcleaner.onHistoryChanged(() => {
