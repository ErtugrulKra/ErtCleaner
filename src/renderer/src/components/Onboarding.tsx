...
export function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const applyAndFinish = async () => {
    ...
    try {
      ...
      await window.ertcleaner?.settingsSet?.(settingsPayload)
      await window.ertcleaner?.applyStartup?.(settings.runAtStartup).catch(() => {})
      window.ertcleaner?.applyTray?.(settings.minimizeToTray)
    } catch {
      // Best-effort
    }
  }
  ...
  return (
    <StepWrapper>
      <div className="flex flex-col items-center text-center">
        <img src={logoSrc} alt="ErtCleaner" className="mb-5 h-20 w-20 rounded-2xl" />
      ...
      <div>
        ...
        <div className="space-y-1">
          <SettingRow
            ...
            desc={t('runAtStartupDescriptionWindows')}
