...
export function DiskAnalyzerPage() {
  ...
  useEffect(() => {
    if (drives.length === 0) {
      window.ertcleaner?.diskDrives?.().then(store.setDrives).catch((err) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await window.ertcleaner.diskAnalyze(selectedDrive)
      store.setData(result); store.setBreadcrumb([result])
    } catch (err) {
      console.error('Disk analysis failed:', err)
      toast.error(t('failedToAnalyzeToastWindows', { drive: selectedDrive }), { description: t('failedToAnalyzeDescMakeAccessible') })
      store.setError(t('failedToAnalyzeErrorWindows', { drive: selectedDrive }))
    }
  ...
  const handleFileTypeScan = async () => {
    ...
    try {
      const result = await window.ertcleaner.diskFileTypes(selectedDrive)
      store.setFileTypes(result)
    } catch (err) {
      console.error('File type scan failed:', err)
      store.setError(t('failedToScanFileTypesWindows', { drive: selectedDrive }))
    }
  ...
  return (
    <div className="animate-fade-in">
      ...
      {analyzing && <ScanProgress status="scanning" progress={0} currentPath={t('analyzingProgressWindows', { drive: selectedDrive })} className="mb-5" />}
      ...
      {data && (
        <>
          ...
          {viewMode === 'filetypes' && (
            <>
              {fileTypesLoading && <ScanProgress status="scanning" progress={0} currentPath={t('scanningFileTypesWindows', { drive: selectedDrive })} className="mb-5" />}
