...
export function UninstallerPage() {
  ...
  useEffect(() => {
    const cleanup = window.ertcleaner.onUninstallerProgress((data: UninstallProgress) => {
  ...
  const handleLoad = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.uninstallerList()
  ...
  const handleUninstall = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.uninstallerUninstall(program.id)
      const s = useUninstallerStore.getState()
  ...
  const handleForceRemove = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.uninstallerForceRemove(program.id)
      const s = useUninstallerStore.getState()
      s.setUninstallResult(result)
      s.setProgress(null)
