...
const categories: CategoryDef[] = [
  ...
  },
  {
    ...
    icon: Eye,
    color: '#2f7de1',
    bg: 'rgba(47,125,225,0.08)',
    border: 'rgba(47,125,225,0.15)'
  },
  {
    id: 'tasks',
...
export function PrivacyShieldPage({ embedded }: { embedded?: boolean }) {
  ...
  const handleScan = useCallback(async () => {
    ...
    progressCleanupRef.current = window.ertcleaner.onPrivacyProgress?.((data) => {
      usePrivacyStore.getState().setProgress(data)
    }) ?? null

    try {
      const result = await window.ertcleaner.privacyScan()
  ...
  const handleApplyAll = useCallback(async () => {
    ...
    store.setApplyResult(null)
    try {
      const result = await window.ertcleaner.privacyApply(unprotectedIds)
      usePrivacyStore.getState().setApplyResult(result)
      // Re-scan to get updated state
      const updated = await window.ertcleaner.privacyScan()
  ...
  const handleApplyCategory = useCallback(async (categoryId: string) => {
    ...
    store.setApplyResult(null)
    try {
      const result = await window.ertcleaner.privacyApply(ids)
      usePrivacyStore.getState().setApplyResult(result)
      const updated = await window.ertcleaner.privacyScan()
      usePrivacyStore.getState().setState(updated)
      usePrivacyStore.getState().setStatus('done')
  ...
  const handleToggleSingle = useCallback(async (settingId: string) => {
    ...
    try {
      const result = isEnabling
        ? await window.ertcleaner.privacyApply([settingId])
        : await window.ertcleaner.privacyRevert([settingId])
      const updated = await window.ertcleaner.privacyScan()
      usePrivacyStore.getState().setState(updated)
      usePrivacyStore.getState().setStatus('done')
