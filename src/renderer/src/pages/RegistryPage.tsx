...
const typeColors: Record<CardType, { bg: string; text: string }> = {
  ...
  performance: { bg: 'rgba(47,125,225,0.1)', text: '#2f7de1' },
...
function RegistryPageContent() {
  ...
  useEffect(() => {
    const cleanup = window.ertcleaner.onRegistryFixProgress((data) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    store.setError(null)
    try {
      const results = await window.ertcleaner.registryScan()
  ...
  const handleScanCancel = useCallback(async () => {
    try {
      await window.ertcleaner.registryScanCancel()
    } catch { /* ignore */ }
    useRegistryStore.getState().setScanning(false)
  }, [])
  ...
  const handleFixCancel = useCallback(async () => {
    try {
      await window.ertcleaner.registryFixCancel()
    } catch { /* ignore */ }
  ...
  const handleFix = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.registryFix(selectedIds)
