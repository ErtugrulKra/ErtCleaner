...
export function StartupPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      const list = await window.ertcleaner.startupList()
  ...
  const loadBootTrace = useCallback(async () => {
    ...
    try {
      const trace = await window.ertcleaner.startupBootTrace()
  ...
  const handleToggle = async (item: StartupItem, enabled: boolean) => {
    ...
    try {
      const success = await window.ertcleaner.startupToggle(item.name, item.location, item.command, item.source, enabled)
      if (!success) {
  ...
  const handleDelete = async (item: StartupItem) => {
    const startTime = Date.now()
    try {
      const success = await window.ertcleaner.startupDelete(item.name, item.source === 'startup-folder' ? item.command : item.location, item.source)
      if (success) {
