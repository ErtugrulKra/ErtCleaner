...
const typeConfigBase: Record<HistoryEntryType, { labelKey: string; icon: typeof Sparkles; color: string; bg: string }> = {
  ...
  privacy: { labelKey: 'typeLabels.privacy', icon: ShieldCheck, color: '#2f7de1', bg: 'rgba(47,125,225,0.1)' },
...
const PIE_COLORS = ['#f59e0b', '#3b82f6', '#22c55e', '#a855f7', '#ec4899', '#2f7de1', '#ef4444', '#6366f1']
...
export function HistoryPage() {
  ...
  const typeBreakdown = useMemo(() => {
    ...
    for (const e of entries) {
      const label = typeConfig[e.type]?.label ?? e.type
...
function DeletedFilesSection({ from, to }: { from: string; to: string }) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    // origin: a CLI-triggered clean can overlap this run's window; those
    // deletions belong to the CLI history, not to this entry.
    window.ertcleaner.deletionLogQuery({ from, to, origin: 'local', offset: 0, limit: DELETED_FILES_PAGE })
      .then((page) => {
        if (cancelled) return
  ...
  const loadMore = async () => {
    setLoadingMore(true)
    try {
      const page = await window.ertcleaner.deletionLogQuery({
  ...
  const handleExport = async () => {
    setExporting(true)
    try {
      const savedTo = await window.ertcleaner.deletionLogExport({ from, to, origin: 'local' })
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        {total > 0 && (
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <IconButton icon={FolderOpen} label={t('detail.deletedFiles.openLog')} onClick={() => window.ertcleaner.deletionLogReveal()} />
          </div>
        )}
      </div>
