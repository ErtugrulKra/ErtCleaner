...
export function GameModePage() {
  ...
  const handleActivate = useCallback(async () => {
    ...
    progressCleanupRef.current = window.ertcleaner?.onGameModeProgress?.((data) => {
      useGameModeStore.getState().setProgress(data)
    }) ?? null

    try {
      const result = await window.ertcleaner.gameModeActivate(config)
  ...
  const handleDeactivate = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.gameModeDeactivate()
  ...
  const handleDiscardPending = useCallback(async () => {
    try {
      const result = await window.ertcleaner.gameModeDiscardPending()
      if (!result.discarded) {
