...
/** Windows managers ErtCleaner can aggregate, with their display labels. */
...
export function SoftwareUpdaterPage({ embedded }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    const cleanup = window.ertcleaner.onSoftwareUpdateProgress((data: UpdateProgress) => {
  ...
  const handleUpdate = useCallback(
    async (appsToUpdate: UpdatableApp[]) => {
      ...
      try {
        const result = await window.ertcleaner.softwareUpdateRun(items)
        const s = useUpdaterStore.getState()
  ...
  const handleToggleManager = useCallback(
    async (manager: WindowsPackageManager) => {
      ...
      await window.ertcleaner.settingsSet({ windowsPackageManagers: next })
      handleCheck()
    },
    [handleCheck],
  )
  ...
  return (
    <div className={embedded ? '' : 'animate-fade-in'}>
      ...
      <div className="mb-5 flex items-center gap-2.5">
        ...
        {/* Package manager toggles — aggregate across managers */}
        <div
