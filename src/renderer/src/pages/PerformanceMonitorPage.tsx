...
export function PerformanceMonitorPage() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        const [info, disks] = await Promise.all([
          window.ertcleaner.perfGetSystemInfo(),
          window.ertcleaner.perfGetDiskHealth()
        ])
        setSystemInfo(info)
        setDiskHealth(disks)

        snapshotUnsub = window.ertcleaner.onPerfSnapshot((data) => {
          pushSnapshot(data)
        })

        processUnsub = window.ertcleaner.onPerfProcessList((data) => {
          setProcessList(data.processes, data.totalCount)
        })

        await window.ertcleaner.perfStartMonitoring()
        setMonitoring(true)
      } catch {
    ...
    return () => {
      snapshotUnsub?.()
      processUnsub?.()
      window.ertcleaner.perfStopMonitoring().catch(() => {})
      reset()
    }
  ...
  const togglePause = useCallback(async () => {
    if (paused) {
      ...
      await window.ertcleaner.perfStopMonitoring()
      setPaused(true)
    }
  }, [paused])
