...
export function SchedulesPage() {
  ...
  const save = (updated: ScheduleEntry[]) => {
    ...
    window.ertcleaner?.settingsSet?.({ schedules: updated }).catch(() => {})
  }
  ...
  const ensureBackgroundMode = () => {
    if (!settings.runAtStartup) {
      ...
      window.ertcleaner?.settingsSet?.({ runAtStartup: true }).catch(() => {})
      window.ertcleaner?.applyStartup?.(true).catch(() => {
        updateSettings({ runAtStartup: false })
        window.ertcleaner?.settingsSet?.({ runAtStartup: false }).catch(() => {})
        toast.error(t('failedEnableStartup'))
      })
    }
    if (!settings.minimizeToTray) {
      updateSettings({ minimizeToTray: true })
      window.ertcleaner?.settingsSet?.({ minimizeToTray: true }).catch(() => {})
      window.ertcleaner?.applyTray?.(true)
    }
  }
