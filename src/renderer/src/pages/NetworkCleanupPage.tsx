...
export function NetworkCleanupPage() {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.networkScan()
  ...
  const handleClean = useCallback(async () => {
    ...
    try {
      ...
      const result = await window.ertcleaner.networkClean([...currentSelectedIds])
      const s = useNetworkStore.getState()
      ...
      try {
        const freshItems = await window.ertcleaner.networkScan()
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title={t('pageTitle')}
        description={t('pageDescriptionWindows')}
        action={
      ...
      <ConfirmDialog
        ...
        description={`${t('confirmDescription', { count: selectedIds.size })}${selectedIds.size > 0 && items.some((i) => i.type === 'wifi-profile' && selectedIds.has(i.id)) ? ' ' + t('confirmWifiWarning') : ''}${items.some((i) => i.type === 'network-history' && selectedIds.has(i.id)) ? ' ' + t('confirmNetworkHistoryWarning') : ''}`}
