...
export function DriverManagerPage({ embedded }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    const cleanupDriver = window.ertcleaner.onDriverProgress((data: DriverScanProgress) => {
      useDriverStore.getState().setScanProgress(data)
    })
    const cleanupUpdate = window.ertcleaner.onDriverUpdateProgress((data: DriverUpdateProgress) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    const [staleResult, updateResult] = await Promise.allSettled([
      window.ertcleaner.driverScan(),
      window.ertcleaner.driverUpdateScan()
    ])
  ...
  const handleApply = useCallback(async () => {
    ...
    if (selectedUpdates.length > 0) {
      ...
      try {
        const result = await window.ertcleaner.driverUpdateInstall(ids)
    ...
    if (selectedStale.length > 0) {
      ...
      try {
        const result = await window.ertcleaner.driverClean(names)
