...
export function EmptyFolderCleanerPage() {
  ...
  useEffect(() => {
    if (!window.ertcleaner?.onEmptyFoldersProgress) return
    return window.ertcleaner.onEmptyFoldersProgress((data) => {
  ...
  const handleSelectDir = async () => {
    const dir = await window.ertcleaner?.emptyFoldersSelectDir?.()
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await window.ertcleaner?.emptyFoldersScan?.({
  ...
  const handleCancel = async () => {
    await window.ertcleaner?.emptyFoldersCancel?.()
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await window.ertcleaner?.emptyFoldersDelete?.(paths, store.deleteMode)
      if (result) {
                ...
                {store.result.folders.map((folder, idx) => (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        window.ertcleaner?.emptyFoldersOpenLocation?.(folder.path)
                      }}
