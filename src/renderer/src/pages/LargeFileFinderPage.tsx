...
export function LargeFileFinderPage() {
  ...
  useEffect(() => {
    if (!window.ertcleaner?.onLargeFilesProgress) return
    return window.ertcleaner.onLargeFilesProgress((data) => {
  ...
  const handleSelectDir = async () => {
    const dir = await window.ertcleaner?.largeFilesSelectDir?.()
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await window.ertcleaner?.largeFilesScan?.({
  ...
  const handleCancel = async () => {
    await window.ertcleaner?.largeFilesCancel?.()
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await window.ertcleaner?.largeFilesDelete?.(paths, store.deleteMode)
      if (result) {
                ...
                {store.result.files.map((file, idx) => (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        window.ertcleaner?.largeFilesOpenLocation?.(file.path)
                      }}
