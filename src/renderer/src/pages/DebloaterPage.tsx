...
const categoryColors: Record<BloatwareApp['category'], { bg: string; text: string; labelKey: string }> = {
  ...
  communication: { bg: 'rgba(47,125,225,0.1)', text: '#2f7de1', labelKey: 'debloater.categoryCommunication' },
...
export function DebloaterPage({ embedded }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    const cleanup = window.ertcleaner.onDebloaterRemoveProgress((data) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      const results = await window.ertcleaner.debloaterScan()
  ...
  const handleRemove = useCallback(async () => {
    ...
    try {
      const result = await window.ertcleaner.debloaterRemove(selectedPkgs)
