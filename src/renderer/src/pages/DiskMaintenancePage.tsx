...
function StatusPill({ status, reason }: { status: TrimStatus; reason: string }) {
  ...
  const styles: Record<TrimStatus, { bg: string; color: string; label: string }> = {
    'recently-trimmed': { bg: 'rgba(34,197,94,0.12)', color: '#22c55e', label: t('trimStatusRecentlyTrimmed') },
    'ok': { bg: 'rgba(63,63,70,0.4)', color: 'var(--text-muted)', label: t('trimStatusOk') },
    'recommended': { bg: 'rgba(245,158,11,0.12)', color: '#f59e0b', label: t('trimStatusRecommended') },
    'not-applicable': { bg: 'rgba(63,63,70,0.4)', color: 'var(--text-muted)', label: t('trimStatusNotApplicable') },
    'disabled': { bg: 'rgba(239,68,68,0.12)', color: '#ef4444', label: t('trimStatusDisabled') },
    'unknown': { bg: 'rgba(63,63,70,0.4)', color: 'var(--text-muted)', label: t('trimStatusUnknown') },
  }
...
export function DiskMaintenancePage() {
  ...
  useEffect(() => {
    if (!window.ertcleaner?.onDiskTrimProgress) return
    return window.ertcleaner.onDiskTrimProgress((data) => store.setProgress(data))
  }, [])
  ...
  async function refresh(): Promise<void> {
    ...
    store.setError(null)
    try {
      const list = await window.ertcleaner.diskTrimList()
  ...
  async function handleRun(): Promise<void> {
    ...
    try {
      const results = await window.ertcleaner.diskTrimRun(selectableSelected)
