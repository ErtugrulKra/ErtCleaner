...
const WIN11_NOTICE_KEY = 'ertcleaner.contextMenu.win11Notice.dismissed'
...
const SOURCE_PILL_COLOR: Record<ContextMenuSource, { bg: string; text: string }> = {
  ...
  'Microsoft':    { bg: 'rgba(47,125,225,0.10)',  text: '#4b93ee' },
  'Windows':      { bg: 'rgba(47,125,225,0.10)',  text: '#4b93ee' },
...
const GROUP_PALETTE: ReadonlyArray<{ bg: string; text: string }> = [
  ...
  { bg: 'rgba(47,125,225,0.10)',  text: '#4b93ee' },
]
...
function ContextMenuCleanerPageContent() {
  ...
  useEffect(() => {
    const cleanup = window.ertcleaner.onContextMenuApplyProgress((data) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    store.setError(null)
    try {
      const result = await window.ertcleaner.contextMenuScan()
  ...
  const handleScanCancel = useCallback(async () => {
    try { await window.ertcleaner.contextMenuScanCancel() } catch { /* ignore */ }
    useContextMenuStore.getState().setScanning(false)
  }, [])
  ...
  const handleApply = useCallback(async (action: ContextMenuAction, requests?: ContextMenuApplyRequest[]) => {
    ...
    try {
      const result = await window.ertcleaner.contextMenuApply(reqs)
    ...
    <div className="animate-fade-in">
      ...
      {requiresAdminInSelection && selectedCount > 0 && !applying && (
        <div className="mb-3 flex items-center gap-2.5 rounded-xl px-4 py-2.5 text-[12px]"
          ...
          <button
            onClick={() => window.ertcleaner.elevationRelaunch?.().catch(() => {})}
