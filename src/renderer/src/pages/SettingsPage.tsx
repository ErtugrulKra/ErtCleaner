...
export function SettingsPage() {
  ...
  useEffect(() => { window.ertcleaner?.settingsGet?.().then(setSettings).catch(() => {}) }, [])

  const save = (partial: Partial<typeof settings>) => {
    updateSettings(partial)
    window.ertcleaner?.settingsSet?.(partial).catch(() => {})
  }

  const saveStartup = async (enabled: boolean) => {
    save({ runAtStartup: enabled })
    try {
      await window.ertcleaner?.applyStartup?.(enabled)
    } catch {
  ...
  const saveTray = (enabled: boolean) => {
    ...
    window.ertcleaner?.applyTray?.(enabled)
  }
    ...
    <div className="feature-page settings-page animate-fade-in">
      ...
      <Section title={t('sectionBackups', 'Backups')}>
        <BackupFolderRow
          ...
          onPick={async () => {
            const picked = await window.ertcleaner?.settingsSelectBackupDir?.()
            if (picked) {
          ...
          onOpen={() => { window.ertcleaner?.settingsOpenBackupDir?.().catch(() => {}) }}
        ...
        <div className="space-y-2 pb-3">
          ...
          <div className="flex items-center gap-2.5">
            <input type="text" value={newExclusion} onChange={(e) => setNewExclusion(e.target.value)}
              ...
              placeholder={t('exclusionPlaceholderWindows')}
...
function BackupFolderRow({
  ...
  const displayPath = isCustom ? path : t('backupFolderDefaultLabel', 'Default (Documents/ErtCleaner Backups)')
  return (
    <div className="space-y-3">
      <div>
        <p className="text-[13px] font-medium text-zinc-300">{t('backupFolderLabel', 'Backup folder')}</p>
        <p className="mt-0.5 text-[12px]" style={{ color: 'var(--text-muted)' }}>
          {t('backupFolderDesc', 'Where ErtCleaner writes registry and shell-extension backups before making changes. Existing backups stay in their previous location when you switch folders.')}
        </p>
      </div>
      <div className="flex items-center gap-2.5">
        <div
