...
export function FileShredderPage() {
  ...
  useEffect(() => {
    if (!window.ertcleaner?.onShredderProgress) return
    return window.ertcleaner.onShredderProgress((data) => {
  ...
  const handleAddFiles = async () => {
    const entries = await window.ertcleaner?.shredderSelectFiles?.()
    if (entries?.length) store.addEntries(entries)
  }

  const handleAddFolders = async () => {
    const entries = await window.ertcleaner?.shredderSelectFolders?.()
    if (entries?.length) store.addEntries(entries)
  }
  ...
  const handleShred = async () => {
    ...
    try {
      ...
      const result = await window.ertcleaner?.shredderShred?.(paths)
      if (result) {
  ...
  const handleCancel = async () => {
    await window.ertcleaner?.shredderCancel?.()
  }
          ...
          >
            {store.entries.map((entry, idx) => (
              ...
              >
                ...
                <button
                  onClick={() => window.ertcleaner?.shredderOpenLocation?.(entry.path)}
