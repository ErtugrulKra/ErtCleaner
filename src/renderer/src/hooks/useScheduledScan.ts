...
const CLEANER_TASKS: Record<string, {
  ...
  'cleaner:system': {
    label: 'System',
    scan: () => window.ertcleaner.systemScan(),
    clean: (ids) => window.ertcleaner.systemClean(ids)
  },
  'cleaner:browsers': {
    label: 'Browsers',
    scan: () => window.ertcleaner.browserScan(),
    clean: (ids) => window.ertcleaner.browserClean(ids)
  },
  'cleaner:apps': {
    label: 'Applications',
    scan: () => window.ertcleaner.appScan(),
    clean: (ids) => window.ertcleaner.appClean(ids)
  },
  'cleaner:gaming': {
    label: 'Gaming',
    scan: () => window.ertcleaner.gamingScan(),
    clean: (ids) => window.ertcleaner.gamingClean(ids)
  },
  'cleaner:recycleBin': {
    label: 'Recycle Bin',
    scan: () => window.ertcleaner.recycleBinScan(),
    clean: () => window.ertcleaner.recycleBinClean()
  },
  'cleaner:databases': {
    label: 'Databases',
    scan: () => window.ertcleaner.databaseScan(),
    clean: (ids) => window.ertcleaner.databaseClean(ids)
  }
}
  ...
  try {
    ...
    if (payload.autoApply && cleanerTasks.length > 0) {
      ...
      if (createRestorePoint) {
        try {
          await window.ertcleaner.createRestorePoint(`ErtCleaner zamanlanmış temizlik — ${payload.scheduleName}`)
        } catch {
    ...
    if (payload.tasks.includes('registry')) {
      try {
        ...
        if (payload.autoApply && found > 0) {
          ...
          try {
            const result = await window.ertcleaner.registryFix(ids)
    ...
    if (payload.tasks.includes('drivers')) {
      try {
        ...
        if (payload.autoApply && found > 0) {
          ...
          try {
            const installResult = await window.ertcleaner.driverUpdateInstall(ids)
    ...
    if (payload.tasks.includes('software-update')) {
      try {
        ...
        if (payload.autoApply && found > 0) {
          ...
          try {
            const updateResult = await window.ertcleaner.softwareUpdateRun(items)
    ...
    window.ertcleaner.notifyScheduledScanComplete?.(totalSize, totalItems)
    window.ertcleaner.scheduleRunComplete?.(payload.scheduleId, status)
    refreshSettings()
...
export function useScheduledScan(): void {
  ...
  useEffect(() => {
    if (!window.ertcleaner?.onScheduleRunTrigger) return undefined
    ...
    const processQueue = async () => {
      while (queueRef.current.length > 0) {
        ...
        try {
          ...
          if (!idle) {
            window.ertcleaner.scheduleRunComplete?.(next.scheduleId, 'failed')
    ...
    const unsubscribe = window.ertcleaner.onScheduleRunTrigger((payload: ScheduleRunPayload) => {
